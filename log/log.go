...
func (logger *Logger) doPrintf(level int, printLevel string, format string, a ...interface{}) {
	...
	buf := logger.nextBuff()
	buf.Locker()
	color := levelsColor[level] //(修改者:jq)
	buf.Reset()
	buf.AppendString(fmt.Sprintf("\x1b[%dm", color)) //(修改者:jq)
	buf.AppendString("[")                            //(修改者:jq)
	logger.formatHeader(buf, 3, &now)
	buf.AppendString("]") //(修改者:jq)
	buf.AppendString(printLevel)
	buf.AppendString("\x1b[0m") //(修改者:jq)
